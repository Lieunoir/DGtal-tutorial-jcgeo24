...
void createShape(std::string polynomial, double h, double reach) {
  ...
  auto normals = Estimator == 0 ? trivial_normals
                 : Estimator == 1
                     ? SHG3::getCTrivialNormalVectors(surface, surfels, params)
  ...
  psSmoothMesh =
      polyscope::registerSurfaceMesh("smooth surface", ppositions, faces);

  // Compute curvatures and estimate reach.
  auto curvatures =
      SHG3::getPrincipalCurvaturesAndDirections(shape, K, surfels, params);
...
void myCallback() {
  ...
  if (ImGui::Button("Torus"))
    createShape("torus", GridStep, 2.0);
  ImGui::SameLine();
  if (ImGui::Button("Goursat"))
    createShape("goursat", GridStep, 2.0);
  if (ImGui::Button("Leopold"))
    createShape("leopold", GridStep, 0.5);
  ImGui::SameLine();
  if (ImGui::Button("Goursat-H"))
    createShape("goursat-hole", GridStep, 1.0);
  ImGui::SameLine();
  if (ImGui::Button("Cylinder"))
    createShape("x^2-2*x*y+y^2+z^2-25", GridStep, 5.0);
  ImGui::Text("Reach is at most %f", Reach);
  ImGui::SliderFloat("Gridstep h parameter", &GridStep, 0.025, 2.0);
  ImGui::Text("Normal estimator: ");
  ImGui::SameLine();
  ImGui::RadioButton("Trivial", &Estimator, 0);
  ImGui::SameLine();
  ImGui::RadioButton("CTrivial", &Estimator, 1);
  ImGui::SameLine();
